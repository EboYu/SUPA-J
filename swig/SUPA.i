...
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/MemoryModel/PAGBuilder.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/MemoryModel/ConditionalPT.h"
// #include "DDA/DDAVFSolver.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/PathCondAllocator.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/MSSA/SVFG.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/WPA/Andersen.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/MemoryModel/PointsToDS.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/GraphPrinter.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/MSSA/SVFGBuilder.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/Casting.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/ThreadAPI.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/ExtAPI.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/DataFlowUtil.h"
// #include "DDA/DDAStat.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/DPItem.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/SCC.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/VFG.h"
// #include "Util/SVFModule.h"
// #include "Util/SVFUtil.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/PTACallGraph.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/WPA/CSC.h"
// #include "Util/PTAStat.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/MemoryModel/PAG.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/GEPTypeBridgeIterator.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/MemoryModel/PointsToDFDS.h"
// #include "DDA/DDAClient.h"
// #include "Util/BasicTypes.h"
// #include "MemoryModel/PointerAnalysis.h"
#include "/home/yinbo/disk/workspace/SUPA-J/SVF/include/Util/CPPUtil.h"
...
namespace std {
%template(DPTItemSet) set<DPItem>;
%template(ConstSVFGEdgeSet) set<const SVFGEdge*>;
%template(DPItemToCPtSetMap) map<DPItem, CPtSet>;
%template(DPMToCVarMap) map<DPItem,CVar>;
%template(DPMToDPMMap) map<DPItem,DPItem>;
...
%rename(__eq__) CondVar::operator==;
%rename(__ne__) CondVar::operator!=;
%rename(__as__) CondVar::operator=;
%rename(__ls__) CondVar::operator<;
%rename(__eq__) CondStdSet::operator==;
%rename(__ne__) CondStdSet::operator!=;
%rename(__as__) CondStdSet::operator=;
%rename(__ls__) CondStdSet::operator<;
%rename(__oreq__) CondStdSet::operator|=;
%rename(__andeq__) CondStdSet::operator&=;
%rename(begin_const) CondStdSet::begin() const;
%rename(end_const) CondStdSet::end() const;
...
// %template(CVar) CondVar<Cond>;
typedef CondVar<Cond> CondPTAImpl::CVar;
// %template(CPtSet) CondStdSet<CVar>;
typedef CondStdSet<CVar> CondPTAImpl::CPtSet;
 ...
 */
template<class CVar, class CondPTAImpl::CPtSet, class DPItem>
class DDAVFSolver  {
...
public:
    ...
    typedef std::set<DPItem> DPTItemSet;
    typedef std::map<DPItem, CPtSet> DPItemToCPtSetMap;
    typedef std::map<DPItem,CVar> DPMToCVarMap;
    typedef std::map<DPItem,DPItem> DPMToDPMMap;
    ...
    virtual inline DPItem getDPItem(const CVar& var, const SVFGNode* loc) const {
        DPItem dpm(var,loc);
        return dpm;
    }
    /// Union pts
    ...
    /// Compute points-to
    virtual const CPtSet& findPT(const DPItem& dpm) {
...
protected:
    ...
    virtual void handleSingleStatement(const DPItem& dpm, CPtSet& pts) {
        ...
        if(SVFUtil::isa<AddrSVFGNode>(node)) {
            ...
            for(typename CPtSet::iterator it = loadpts.begin(), eit = loadpts.end(); it!=eit; ++it) {
                backtraceAlongIndirectVF(pts,getDPItemWithOldCond(dpm,*it,node));
            }
        }
            ...
            if(isMustAlias(getLoadDpm(dpm),dpm)) {
                ...
                for(typename CPtSet::iterator it = storepts.begin(), eit = storepts.end(); it!=eit; ++it) {
                    if(propagateViaObj(*it,getLoadCVar(dpm))) {
                        backtraceToStoreSrc(pts,getDPItemWithOldCond(dpm,*it,node));
    ...
    void reCompute(const DPItem& dpm) {
    ...
    void reComputeForEdges(const DPItem& dpm, const SVFGEdgeSet& edgeSet, bool indirectCall = false) {
    ...
    void backtraceAlongIndirectVF(CPtSet& pts, const DPItem& oldDpm) {
    ...
    void backtraceAlongDirectVF(CPtSet& pts, const DPItem& oldDpm) {
        const SVFGNode* node = oldDpm.getLoc();
        const SVFGEdgeSet edgeSet(node->getInEdges());
        for (SVFGNode::const_iterator it = edgeSet.begin(), eit = edgeSet.end(); it != eit; ++it) {
    ...
    ///@{
    inline void startNewPTCompFromLoadSrc(CPtSet& pts, const DPItem& oldDpm) {
    ...
    inline void startNewPTCompFromStoreDst(CPtSet& pts, const DPItem& oldDpm) {
    ...
    inline void backtraceToStoreSrc(CPtSet& pts, const DPItem& oldDpm) {
        const StoreSVFGNode* store = SVFUtil::cast<StoreSVFGNode>(oldDpm.getLoc());
    ...
    virtual void backwardPropDpm(CPtSet& pts, NodeID ptr,const DPItem& oldDpm,const SVFGEdge* edge) {
        DPItem dpm(oldDpm);
    ...
    virtual bool isMustAlias(const DPItem& loadDpm, const DPItem& storeDPm) {
        return false;
    }
    ...
    void resolveFunPtr(const DPItem& dpm) {
    ...
    virtual void handleAddr(CPtSet& pts,const DPItem& dpm,const AddrSVFGNode* addr) = 0;
    /// Get conservative points-to results when the query is out of budget
    virtual CPtSet getConservativeCPts(const DPItem& dpm) = 0;
    /// Handle condition for context or path analysis (backward analysis)
    virtual inline bool handleBKCondition(DPItem& oldDpm, const SVFGEdge* edge) {
        return true;
    }
    /// Update call graph
    virtual inline void updateCallGraphAndSVFG(const DPItem& dpm,CallSite cs,SVFGEdgeSet& svfgEdges) {}
    //@}

    ///Visited flags to avoid cycles
    //@{
    inline void markbkVisited(const DPItem& dpm) {
        backwardVisited.insert(dpm);
    }
    inline bool isbkVisited(const DPItem& dpm) {
        return backwardVisited.find(dpm)!=backwardVisited.end();
    }
    inline void clearbkVisited(const DPItem& dpm) {
    ...
    //@{
    virtual inline CPtSet& getCachedPointsTo(const DPItem& dpm) {
        if (isTopLevelPtrStmt(dpm.getLoc()))
            return getCachedTLPointsTo(dpm);
        else
            return getCachedADPointsTo(dpm);
    }
    virtual inline void updateCachedPointsTo(const DPItem& dpm, CPtSet& pts) {
    ...
    virtual inline CPtSet& getCachedTLPointsTo(const DPItem& dpm) {
        return dpmToTLCPtSetMap[dpm];
    }
    virtual inline CPtSet& getCachedADPointsTo(const DPItem& dpm) {
    ...
    virtual inline DPItem getDPItemWithOldCond(const DPItem& oldDpm,const CVar& var, const SVFGNode* loc) {
        DPItem dpm(oldDpm);
...
private:
    ...
    inline void addDpmToLoc(const DPItem& dpm) {
        locToDpmSetMap[dpm.getLoc()->getId()].insert(dpm);
    }
    inline void removeDpmFromLoc(const DPItem& dpm) {
...
protected:
    ...
    //@{
    inline void addLoadDpmAndCVar(const DPItem& dpm,const DPItem& loadDpm,const CVar& loadVar) {
        addLoadCVar(dpm,loadVar);
        addLoadDpm(dpm,loadDpm);
    }
    /// Note that simply use "dpmToloadDpmMap[dpm]=loadDpm", requires DPItem have a default constructor
    inline void addLoadDpm(const DPItem& dpm,const DPItem& loadDpm) {
    ...
    inline const DPItem& getLoadDpm(const DPItem& dpm) const {
        typename DPMToDPMMap::const_iterator it = dpmToloadDpmMap.find(dpm);
        assert(it!=dpmToloadDpmMap.end() && "not found??");
        return it->second;
    }
    inline void addLoadCVar(const DPItem& dpm, const CVar& loadVar) {
    ...
    inline const CVar& getLoadCVar(const DPItem& dpm) const {
    ...
    inline void handleOutOfBudgetDpm(const DPItem& dpm) {}
    inline bool testOutOfBudget(const DPItem& dpm) {
        if(outOfBudgetQuery) return true;
        if(++ddaStat->_NumOfStep > DPItem::getMaxBudget())
    ...
    inline void addOutOfBudgetDpm(const DPItem& dpm) {
        outOfBudgetDpms.insert(dpm);
    }
    inline bool isOutOfBudgetDpm(const DPItem& dpm) const {
    ...
    inline void addSUStat(const DPItem& dpm, const SVFGNode* node) {
    ...
    inline void rmSUStat(const DPItem& dpm, const SVFGNode* node) {
    ...
    DPItemToCPtSetMap dpmToTLCPtSetMap;	///< points-to caching map for top-level vars
    DPItemToCPtSetMap dpmToADCPtSetMap;	///< points-to caching map for address-taken vars
